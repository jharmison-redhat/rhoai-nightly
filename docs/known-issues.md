...
| `spec.rolloutStrategy` on LLMInferenceService is 3.6+ only — on 3.5 the admission webhooks silently strip it (no error, no effect) | Setting rolloutStrategy on a model does nothing; the field vanishes from the stored object. Single-replica GPU redeploys therefore always surge onto a cold node (cold 40 GB modelcar pull per redeploy) | none needed — field implemented in 3.6 ([kserve `rhoai-3.6`](https://github.com/red-hat-data-services/kserve/tree/rhoai-3.6)/release-v0.17); a 3.5.z backport is unlikely (API stability) | Delete the Deployment and let KServe recreate it — with one GPU node the pod lands on the warm-cache node (details in [issue](issues/llmisvc-rolloutstrategy-silently-pruned.md); don't patch `spec.strategy` — the controller reverts it) |
