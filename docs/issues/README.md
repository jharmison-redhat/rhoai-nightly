...
| [kueue-podsready-5min-eviction.md](kueue-podsready-5min-eviction.md) | **NOT FILED** — no bug to file; fix merged upstream and tracked: [OCPSTRAT-3301](https://redhat.atlassian.net/browse/OCPSTRAT-3301) (Kueue 1.5), commit [2ec61a1](https://github.com/openshift/kueue-operator/commit/2ec61a163a7ec00382251ad8451745e29d16142f) merged 2026-08-21; `stable-v1.4` tops out at v1.4.2 (searched 2026-09-22) | Open — found 2026-09-22 on rhai-tmm (rhods 3.5.1, kueue-operator v1.4.2): qwen3-6-27b-fp8 pods evicted every ~5m in a loop, LLMInferenceService never Ready. Workaround A14 — `gangScheduling` block removed from the Kueue CR, verified same day (2/2, Ready) |
| [llmisvc-rolloutstrategy-silently-pruned.md](llmisvc-rolloutstrategy-silently-pruned.md) | **NOT FILED** — no filing planned: 3.6 implements the field (types + regenerated CRD), gap is 3.5-only, API-field backport to a z-stream unlikely. Nearest: [RHOAIENG-95840](https://redhat.atlassian.net/browse/RHOAIENG-95840) (maps `spec.rolloutStrategy` in the DisaggregatedSet scope — field is real and intended; doesn't track the staleness) | Open — found 2026-09-22 on rhai-tmm (3.5.1 GA + nightly + the `rhoai-3.5` branch tip): `spec.rolloutStrategy` silently stripped by the 4 admission mutators (Go types lack the field — proven live with a control field, not schema pruning). Workaround: operational delete-the-Deployment warm-node redeploy; remove-when = 3.6 upgrade |
...
the D2c file. kueue-podsready-5min-eviction: **no filing** — fix merged
upstream (OCPSTRAT-3301, Kueue 1.5); optional comment on
[OCPKUEUE-862](https://redhat.atlassian.net/browse/OCPKUEUE-862) with the
real-world LLMInferenceService impact (draft in the file).
